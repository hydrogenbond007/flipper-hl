...
def main():
    ...
    address, info, exchange = python_utils.setup(constants.TESTNET_API_URL, skip_ws=True)
...
def fetch_markets():
    """
    Fetches and displays all available markets on Hyperliquid
    """
    # Initialize exchange with None as we don't need authentication for public data
    exchange = Exchange(None, constants.MAINNET_API_URL)
    
    # Fetch all markets
    meta_info = exchange.meta()
    
    if not meta_info or 'universe' not in meta_info:
    ...
    print("-" * 50)
    print(f"{'Symbol':<10} {'Asset Name':<20} {'Price Scale':<15}")
    print("-" * 50)
    
    for market in meta_info['universe']:
        symbol = market['name']
        asset_name = market.get('szDecimals', 'N/A')
        price_scale = market.get('price_scale', 'N/A')
        print(f"{symbol:<10} {asset_name:<20} {price_scale:<15}")
